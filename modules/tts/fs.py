...
from modules.commons.style_modules import get_mask_from_lengths
from modules.tts.style_encoder import MelStyleEncoder
...
class FastSpeech(nn.Module):
    def __init__(self, dict_size, hparams, out_dims=None):
        ...
        # StyleEncoder
        self.style_encoder = MelStyleEncoder(num_mel_bins=hparams["audio_num_mel_bins"], 
                                             style_hidden=hparams["style_hidden"], 
                                             style_vector_dim=self.hidden_size,
                                             style_kernel_size=hparams["style_kernel_size"], 
                                             style_head=hparams["style_head"], 
    ...
    def forward(self, txt_tokens, mel, mel2ph=None, mel_len=None, 
        ...
        # add style vector
        mel_mask = get_mask_from_lengths(mel_len, torch.max(mel_len))
        ret["style_vector"] = style_vector = self.style_encoder.forward_style(mel, mel_mask)

        # add dur
        dur_inp = (encoder_out + style_vector) * src_nonpadding
        ...
        if self.hparams['use_pitch_embed']:
            pitch_inp = (decoder_inp + style_vector) * tgt_nonpadding
            decoder_inp = decoder_inp + self.forward_pitch(pitch_inp, f0, uv, mel2ph, ret, encoder_out)

        # decoder input
        ret['decoder_inp'] = decoder_inp = (decoder_inp + style_vector) * tgt_nonpadding
