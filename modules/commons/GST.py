...
class GST(nn.Module):
    ...
    def __init__(self):
        ...
        self.linear_layer = Linear(hp.gst_size, hp.num_ages)
        self.age_loss = nn.CrossEntropyLoss(ignore_index=-100)

        self.reverse_emotion_classifier = nn.Sequential(
            RevGrad(),
            nn.Linear(hp.gst_size, hp.num_emotions),
            nn.LogSoftmax(),
        )
        self.reverse_emotion_classifier_loss = nn.NLLLoss(ignore_index=-100)
        self.reverse_emotion_classifier_commitment = 0.1

        self.reverse_speaker_classifier = nn.Sequential(
            RevGrad(),
            nn.Linear(hp.gst_size, hp.num_speakers),
            nn.LogSoftmax(),
        )
        self.reverse_speaker_classifier_loss = nn.NLLLoss(ignore_index=-100)
        self.reverse_speaker_classifier_commitment = 0.1

    def forward(self, inputs, age, emotion, speaker):
        enc_out = self.encoder(inputs)
        style_embed = self.stl(enc_out)

        x = style_embed.squeeze(1)
        age = age.squeeze(1)
        emotion = emotion.squeeze(1)
        speaker = speaker.squeeze(1)

        a_x = self.linear_layer(x)  # N, T, M
        feature_age_loss = self.age_loss(a_x, age)
        correct_prediction = (torch.argmax(a_x, 1) == age).sum().float()
        feature_age_acc = correct_prediction / age.size(0) * 100


        e_x = self.reverse_emotion_classifier(x)  # N, T, M
        feature_emotion_loss = self.reverse_emotion_classifier_loss(e_x, emotion) * self.reverse_emotion_classifier_commitment
        correct_emotion_prediction = (torch.argmax(e_x, 1) == emotion).sum().float()
        feature_emotion_acc = correct_emotion_prediction / emotion.size(0) * 100

        s_x = self.reverse_speaker_classifier(x)  # N, T, M
        feature_speaker_loss = self.reverse_speaker_classifier_loss(s_x, speaker) * self.reverse_speaker_classifier_commitment
        correct_speaker_prediction = (torch.argmax(s_x, 1) == speaker).sum().float()
        feature_speaker_acc = correct_speaker_prediction / speaker.size(0) * 100



        return style_embed, feature_age_loss, feature_age_acc, feature_emotion_loss, feature_emotion_acc, feature_speaker_loss, feature_speaker_acc
...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.gru = nn.GRU(input_size=hp.ref_enc_filters[-1] * out_channels,
                          hidden_size=hp.encoder_hidden // 2,
                          batch_first=True)
...
class STL(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.embed = nn.Parameter(torch.FloatTensor(hp.token_num, hp.encoder_hidden // hp.num_heads))
        d_q = hp.encoder_hidden // 2
        d_k = hp.encoder_hidden // hp.num_heads
        # self.attention = MultiHeadAttention(hp.num_heads, d_model, d_q, d_v)
        self.attention = MultiHeadAttention(query_dim=d_q, key_dim=d_k, num_units=hp.gst_size, num_heads=hp.num_heads)
...
class TPSENet(nn.Module):
    ...
    def __init__(self, text_dims, style_dims):
        ...
        self.last = nn.Linear(style_dims, 1)
