...
class FastSpeechTask(SpeechBaseTask):
    ...
    def build_tts_model(self):
        # First step: build a initialized model
        dict_size = len(self.token_encoder)
        self.model = FastSpeech(dict_size, hparams)

    def run_model(self, sample, infer=False, *args, **kwargs):
        # Loading data pairs (text, mel) for training
        txt_tokens = sample['txt_tokens']  # [B, T_t]
        ref_mel = sample['ref_mels']  # [B, T_s, 80]
        ref_mel_len = sample['ref_mel_lengths']
        ...
        if not infer:
            ...
            output = self.model(txt_tokens, mel=ref_mel, mel2ph=mel2ph, mel_len=ref_mel_len, 
            ...
            mel_len, mel2ph, uv, f0 = None, None, None, None
