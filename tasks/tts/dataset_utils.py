...
from modules.commons.slicing_mel import rand_slice_segments
...
class BaseSpeechDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        mel = ref_mel = torch.Tensor(mel)
        mel_len = mel.shape[0]
        # For robust style encoder training
        if self.prefix == "train":
            ref_mel, _ = rand_slice_segments(ref_mel, mel_len)
    ...
    def collater(self, samples):
        ...
        ref_mel_lengths = torch.LongTensor([s['ref_mel'].shape[0] for s in samples]) 
