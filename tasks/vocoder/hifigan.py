...
from modules.vocoder.hifigan.hifigan_only_mrd_c import Generator, DiscriminatorP, DiscriminatorS, \
    generator_loss, feature_loss, discriminator_loss
from modules.vocoder.hifigan.mel_processing import mel_spectrogram_torch
...
class HifiGanTask(VocoderBaseTask):
    def build_model(self):
        ...
        self.model_disc['mpd'] = DiscriminatorP()
        self.model_disc['msd'] = DiscriminatorS()
    ...
    def _training_step(self, sample, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            y_mel = mel_spectrogram_torch(y.squeeze(1), hparams['fft_size'], hparams['audio_num_mel_bins'],
                                          hparams['audio_sample_rate'], hparams['hop_size'], 
                                          hparams['win_size'], hparams['fmin'], hparams['fmax']).transpose(1, 2)
            y_hat_mel = mel_spectrogram_torch(y_.squeeze(1), hparams['fft_size'], hparams['audio_num_mel_bins'],
                                          hparams['audio_sample_rate'], hparams['hop_size'], 
                                          hparams['win_size'], hparams['fmin'], hparams['fmax']).transpose(1, 2)
