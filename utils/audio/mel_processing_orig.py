...
def torch_wav2spec(wav_fn, max_wav_value, fft_size, hop_size, win_length, num_mels, fmin, fmax, sample_rate, center=True):
    ...
    wav = torch.from_numpy(wav) / max_wav_value
    wav = wav.unsqueeze(0)
    if torch.min(wav) < -1.:
        print('min value is ', torch.min(wav))
    if torch.max(wav) > 1.:
    ...
    if fmax_dtype_device not in mel_basis:
        mel = librosa_mel_fn(sr=sample_rate, n_fft=fft_size, n_mels=num_mels, fmin=fmin, fmax=fmax)
        mel_basis[fmax_dtype_device] = torch.from_numpy(mel).to(dtype=wav.dtype, device=wav.device)
    mel = torch.matmul(mel_basis[fmax_dtype_device], spec)
    mel = spectral_normalize_torch(mel)
    spec, mel = spec[:, :, :-1], mel[:, :, :-1]
    # Wav-processing
