...
from utils.audio.mel_processing import torch_wav2spec, save_plot
...
class BaseBinarizer:
    ...
    # divide metadata into n chunks
    def list_chunk(self, len_lst, n):
        lst = []
        for i in range(n):
            lst.append(len_lst//n * i)
        lst.append(len_lst)
        return lst
    ...
    def process_data(self, prefix):
        ...
        chunked_ls = self.list_chunk(len(meta_data), chunk_num)

        print(chunked_ls)
        for i in range(chunk_num):
            
            items = []
            args = [{"item": item} for item in meta_data[chunked_ls[i]:chunked_ls[i+1]]]
            ...
            print(f"······························· {prefix} ({i+1}/{chunk_num})")
        
        # Save binarized files
        builder.finalize()
        np.save(f"{data_dir}/{prefix}_lengths.npy", mel_lengths)
        if len(ph_lengths) > 0:
            np.save(f"{data_dir}/{prefix}_ph_lengths.npy", ph_lengths)
        print(f"| {prefix} total duration: {total_sec:.3f}s, total files: {total_file}")
